...
[![NPM Version](https://img.shields.io/npm/v/@emeraldemperaur/apotheosis-ux.svg)](https://www.npmjs.com/package/@emeraldemperaur/apotheosis-ux)
...
<p align="justify"><strong>Apotheosis UX</strong> is a modern, TypeScript-first component ecosystem engineered to power next generation full-stack web applications. It proffers a comprehensive <strong>Neumorphic design system</strong>, equipping developers with sleek, accessible, and production-ready primitives to craft highly tactile, minimalist, and ultramodern user interfaces. 

Beyond aesthetics, <em>Apotheosis UX</em> pioneers an <strong>AI-ready architecture</strong>—tailored for agent-driven experiences like RAG pipelines, conversational interfaces & generative UI. Built for seamless integration into AI coding environments via the Model Context Protocol (MCP).</p>
...
<p align='justify'>Apotheosis UX proffers an official Model Context Protocol server. This allows AI IDEs to directly read the component registry, type definitions, and backend blueprints to facilitate a hallucination-resistant developer experience.</p>

##### Option A: Remote Enterprise Connection (Frictionless Authentication)
<p align='justify'>For subscribed users, add the managed endpoint to the AI IDE's MCP configuration. Apotheosis UX utilizes Streamable HTTP in tandem with dynamic Clerk OAuth. IDE will automatically prompt a secure browser login with no API keys required.</p>
...
<p align='justify'>Configure the AI IDE to launch Apotheosis UX MCP via <code>stdio</code>:</p>
...
<li><strong>Agentic AI Compatibility:</strong> Strongly typed TypeScript full-stack components annotated with holistic JSDoc context and metadata for LLM (Large Language Model) ingestion.</li>
</ul>

## System Architecture Overview
<p align='justify'>Apotheosis UX ecosystem is structured as a scalable monorepo:</p>
<ol>
<li><strong>Apotheosis UX Component Library</strong> <code>@emeraldemperaur/apotheosis-ux</code>: Named Exports React UI component library.</li>
<li><strong>Apotheosis UX Backend Templates</strong> <code>@emeraldemperaur/apotheosis-ux</code>: Scalable backend service components & architectural templates.</li>
<li><strong>Apotheosis UX Model Context Protocol Server</strong> <code>@emeraldemperaur/apotheosis-ux-mcp</code>: Dual-transport enabled AI context server via remote <code>Streamable HTTP</code> and localhost <code>StdioServerTransport</code>.</li>
</ol>
...
pnpm --filter @emeraldemperaur/apotheosis-ux-mcp publish --access public
```
